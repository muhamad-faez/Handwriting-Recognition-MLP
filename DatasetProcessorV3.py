...
class DatasetProcessorV3:
    def __init__(self, csv_file_path, img_folder_path, output_size=(64, 64)):
        self.csv_file_path = csv_file_path
        self.img_folder_path = img_folder_path
        self.output_size = output_size
        # Transformations for training data with color jitter
        self.train_transform = transforms.Compose([
            transforms.Resize(output_size),
            transforms.RandomAffine(0, scale=(0.8, 1.2)),  # Scaling images by 80% to 120%
            transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.02),  # Adding Color Jitter
            transforms.Grayscale(),
            transforms.ToTensor(),
            transforms.Normalize((0.5,), (0.5,))
        ])
        # Transformations for validation and test data without scaling or color jitter
        ...
        print(f"Initialized DatasetProcessorV3 with images resized to {output_size}.")
